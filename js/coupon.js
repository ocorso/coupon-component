...
proto._onGetActiveCoupons = function(soapResponse){
  ...
  if(isFound) {
    this.clipCoupon(this.currentCouponId, );
  } else console.warning('offer Id not found, show dialog');
...
*/
    proto.clipCoupon = function(couponId, referenceId, partnerId, version) {
        ...
        $.soap({
          ...
          data: {
            'com:in0': [couponId],//couponId from getActiveCouponsByRetailer
            'com:in1': referenceId, //referenceId
            'com:in2': partnerId, //PartnerId
            'com:in3': parseInt(this.version)
          },
          envAttributes: this.envAttributes,
          appendMethodToURL: false, // method name will be appended to URL defaults to true
          soap12: false,
          success: function(soapResponse) {
            ...
            console.log('success');
            response = soapResponse.toJSON();
            //console.log(response['#document']['soap:Envelope']['soap:Body']['ns1:clipCouponResponse']['ns1:out']);
